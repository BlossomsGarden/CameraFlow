...
def compress_int4_weight(weight: torch.Tensor):  # (n, m)
    with torch.cuda.device(weight.device):
        ...
        assert m % 2 == 0
        m = m // 2
        out = torch.empty(n, m, dtype=torch.int8, device="npu")
...
def extract_weight_to_half(weight: torch.Tensor, scale_list: torch.Tensor, source_bit_width: int):
    assert scale_list.dtype in [torch.half, torch.float16]
    ...
    with torch.cuda.device(weight.device):
        n, m = weight.size(0), weight.size(1)
        out = torch.empty(n, m * (8 // source_bit_width), dtype=scale_list.dtype, device="npu")
        stream = torch.cuda.current_stream()
...
class QuantizedLinear(torch.nn.Module):
    def __init__(self, weight_bit_width: int, weight, bias=None, device="npu", dtype=None, empty_init=False):
        super().__init__()
...
class RotaryEmbedding(nn.Module):
    ...
    def forward_impl(
        ...
        if dtype in (torch.float16, torch.float16, torch.int8):
            cache = cache.bfloat16() if dtype == torch.float16 else cache.half()
        return cache
