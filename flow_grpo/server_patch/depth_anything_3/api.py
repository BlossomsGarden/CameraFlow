...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def inference(
        ...
        # Optimization: skip unnecessary computations if only extrinsics are needed
        extrinsics_only: bool = False,
    ) -> Prediction:
        """
        ...
        """
        # Check export format only if it's not None
        ...
        prediction = self._convert_to_prediction(raw_output, extrinsics_only=extrinsics_only)
        ...
        # Skip adding processed images if only extrinsics are needed (optimization)
    ...
    def _convert_to_prediction(self, raw_output: dict[str, torch.Tensor], extrinsics_only: bool = False) -> Prediction:
        """Convert raw model output to Prediction object."""
        start_time = time.time()
        output = self.output_processor(raw_output, extrinsics_only=extrinsics_only)
        end_time = time.time()
