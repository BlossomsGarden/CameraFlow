...
class WanVideoReCamMasterPipeline(BasePipeline):
    ...
    def __init__(self, device="npu", torch_dtype=torch.float16, tokenizer_path=None):
    ...
    def tensor2video(self, frames):
        if frames.dim() == 4:
            ...
            raise ValueError(f"Unsupported frames dimension: {frames.shape}")
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    #     print("has_image_input")
    #     x = torch.cat([x, y], dim=1)  # (b, c_x + c_y, f, h, w)
    #     clip_embdding = dit.img_emb(clip_feature)
    #     context = torch.cat([clip_embdding, context], dim=1)
    

    # f: 时间轴上 patch 的个数（通常等于帧数或帧方向的下采样/分块数）
    # h: 高度方向的 patch 个数
    # w: 宽度方向的 patch 个数
    # 总体上，序列长度 N = f × h × w，对应视频被分块后每个位置一个 token。
    x, (f, h, w) = dit.patchify(x)  
    # 把3D位置编码按轴分解
    # dit.freqs[0] 时间/帧维度
    # dit.freqs[1] 空间高度维度
    # dit.freqs[2] 空间宽度维度的频率/位置编码基
    # freqs 是 3D RoPE 的位置“旋转相位”表：把时间轴、高度轴、宽度轴各自的一维复数相位（cis(θ)=cosθ+i sinθ）预计算好，推理时按实际 f/h/w 截取、广播到 3D 网格并拼接，供自注意力里的 RoPE 直接做复数乘法完成旋转位置编码。
    # 之所以“像提前算好只要加载”：因为这些相位只和维度、最大序列长度有关，不依赖输入数据；预计算一次、多次复用，推理时只做切片+reshape/expand/concat，避免每次现场算三角函数。
