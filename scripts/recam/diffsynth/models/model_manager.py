...
def load_model_from_huggingface_folder(file_path, model_names, model_classes, torch_dtype, device):
    loaded_model_names, loaded_models = [], []
    for model_name, model_class in zip(model_names, model_classes):
        if torch_dtype in [torch.float32, torch.float16, torch.float16]:
...
class ModelDetectorTemplate:
    ...
    def load(self, file_path="", state_dict={}, device="npu", torch_dtype=torch.float16, **kwargs):
        return [], []
...
class ModelDetectorFromPatchedSingleFile:
    ...
    def load(self, file_path="", state_dict={}, device="npu", torch_dtype=torch.float16, model_manager=None, **kwargs):
        if len(state_dict) == 0:
            state_dict = load_state_dict(file_path)
