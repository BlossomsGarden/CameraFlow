...
def sinusoidal_embedding_1d(dim, position):
    sinusoid = torch.outer(
        ...
        torch.pow(10000, -torch.arange(dim//2, dtype=torch.float32, device=position.device).div(dim//2))
    )

    x = torch.cat([torch.cos(sinusoid).to(position.dtype), torch.sin(sinusoid).to(position.dtype)], dim=1)
    return x
...
def precompute_freqs_cis(dim: int, end: int = 1024, theta: float = 10000.0):
    ...
    # freqs = torch.outer(torch.arange(end, dtype=torch.float32, device=freqs.device), freqs)
    # freqs_cis = torch.polar(torch.ones_like(freqs, dtype=torch.float32), freqs.float()).to(torch.complex64)
    # return freqs_cis
    
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim))
    freqs = torch.outer(torch.arange(end, dtype=torch.bfloat16, device=freqs.device), freqs).float()
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, cam_emb, t_mod, freqs):
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (self.modulation.to(dtype=t_mod.dtype, device=t_mod.device) + t_mod).chunk(6, dim=1)
...
class WanModel(torch.nn.Module):
    ...
    # 直观上：把 3D 网格上的每个 patch 映射成一条 token，再配上它在时间/高度/宽度上的位置。
    ...
    def forward(self,
        ...
        **kwargs
        ...
        freqs = torch.cat([
            ...
            w0.view(1, 1, w, -1).expand(f, h, w, -1),
        ...
        # x.shape:  torch.Size([8, 98280, 1536])
        # cam_emb.dtype:  torch.bfloat16
        # cam_emb.shape:  torch.Size([8, 21, 12])
        # context.dtype:  torch.bfloat16
        # context.shape:  torch.Size([1, 512, 1536])
        # t_mod.dtype:  torch.bfloat16
        # t_mod.shape:  torch.Size([1, 6, 1536])
        # freqs.dtype:  torch.complex64
        # freqs.shape:  torch.Size([98280, 1, 64])
        #
        time_start = time.time()
        for block in self.blocks:
            if self.training and use_gradient_checkpointing:
                ...
                # block expects a pair/triple to apply RoPE; we pass a wrapper
                # For efficiency, blocks will pick cos/sin appropriate for the sequence shape
